...
architecture RTL of effect_negative is
    ...
    begin
        process(i_clk50) is
            ...
            --Control saturation
            ---------------------
            r_not_Red8 <= not i_Red8;
            o_Red8 <=(i_Red8 * ( 100- r_intensity) / 100) + (r_not_Red8 * r_intensity/ 100);
            
            r_not_Green8 <= not i_Green8;
            o_Green8 <=(i_Green8 * ( 100- r_intensity) / 100) + (r_not_Green8 * r_intensity/ 100);

            r_not_Blue8 <= not i_Blue8;
            o_Blue8 <=(i_Blue8 * ( 100- r_intensity) / 100) + (r_not_Blue8 * r_intensity/ 100);
