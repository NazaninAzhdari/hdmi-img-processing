...
architecture RTL of effect_fire is
    signal sum_RGB  :   integer     :=0; 
	 signal r_Rx32 : unsigned(7 downto 0) :=(others=>'0'); 
	 signal r_Gx8 : unsigned(7 downto 0) :=(others=>'0'); 
    begin
        ...
        --o_Blue8  <= sum_RGB / 4  (weak Blue)
		  ...
		  r_Rx32 <= to_unsigned((sum_RGB * 20), 8);
		  r_Gx8 <= to_unsigned((sum_RGB * 10) , 8);

        --Strong Red
        o_pixel(23 downto 16) <= r_Rx32
                                when r_Rx32 < 256  
                                else (others=>'1');
        ...
        --Weak Blue
        o_pixel(7 downto 0)   <= shift_right(to_unsigned(sum_RGB , 8) , 2)
                                when shift_right(to_unsigned(sum_RGB , 8) , 2) > 0  
                                else (others=>'0');
