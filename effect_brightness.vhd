...
architecture RTL of effect_brightness is
    ...
    begin
        process(i_clk50) is
            ...
            o_Red8   <= to_unsigned((to_integer(i_Red8) + r_intensity) , 8) 
                        when (to_integer(i_Red8) + r_intensity) < 256  
                        else (others=>'1');

            o_Green8 <= to_unsigned((to_integer(i_Green8) + r_intensity) , 8) 
                        when (to_integer(i_Green8) + r_intensity) < 256  
                        else (others=>'1');

            o_Blue8  <= to_unsigned((to_integer(i_Blue8) + r_intensity) , 8) 
                        when (to_integer(i_Blue8) + r_intensity) < 256  
                        else (others=>'1');
